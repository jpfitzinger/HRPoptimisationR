...
#--------------------------------------------------------------------------
      ...
      get_mean_variance <- function(cov, return_raw, shrink_means = FALSE, robust_cov = FALSE) {
        
        Amat <- cbind(matrix(1,ncol(rets),1), diag(ncol(rets)))
        bvec <- c(1,rep(0,ncol(rets)))
...
#--------------------------------------------------------------------------
      ...
      opt_methods <- list(MVO = c("Mean-Varniance weighting","get_mean_variance(cov, return_raw)"),
